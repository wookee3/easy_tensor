...
def _get_fans(shape):
    ...
    Args:
      shape: A list of integers. (xx,xx, in_dim, out_dim)
    ...
    if len(shape) == 2:
        fan_in = shape[0]
        fan_out = shape[1]
    elif len(shape) == 3 or len(shape) == 4 or len(shape) == 5:
...
def constant(name, shape, value=0, dtype=tf.floatx, summary=True, regularizer=None, trainable=True):
    ...
    Args:
      ...
      dtype: The data type. Only floating point types are supported. Default is floatx.
...
def uniform(name, shape, scale=0.05, dtype=tf.floatx, summary=True, regularizer=None, trainable=True):
...
def he_uniform(name, shape, scale=1, dtype=tf.floatx, summary=True, regularizer=None, trainable=True):
    ...
    Args:
      ...
      dtype: The data type. Default is floatx.
      summary: If True, add this constant to tensor board summary.
      regularizer:  A (Tensor -> Tensor or None) function; the result of applying it on a newly created variable
        will be added to the collection tf.GraphKeys.REGULARIZATION_LOSSES and can be used for regularization
      trainable: If True, add this constant to trainable collection. Default is True.
...
def glorot_uniform(name, shape, scale=1, dtype=tf.floatx, summary=True, regularizer=None, trainable=True):
...
def identity(name, dim, scale=1, dtype=tf.floatx, summary=True, regularizer=None, trainable=True):
...
def orthogonal(name, shape, scale=1.1, dtype=tf.floatx, summary=True, regularizer=None, trainable=True):
    r"""Creates a tensor variable of which initial values are of
    ...
    Args:
      ...
      dtype: Either floatx or float64.
      summary: If True, add this constant to tensor board summary.
      regularizer:  A (Tensor -> Tensor or None) function; the result of applying it on a newly created variable
        will be added to the collection tf.GraphKeys.REGULARIZATION_LOSSES and can be used for regularization
      trainable: If True, add this constant to trainable collection. Default is True.
...
def external(name, value, dtype=tf.floatx, summary=True, regularizer=None, trainable=True):
